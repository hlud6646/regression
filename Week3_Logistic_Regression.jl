...
# ╔═╡ e1274e52-befc-11ee-2685-9339ca59724c
begin
	using CSV
	using DataFrames
	using Distributions
	...
	# Fix some conflicts
	Continuous = MLJ.Continuous
end;

# ╔═╡ 5481b4f2-9ce1-4553-947f-0520e1517124
md"""
# Binary Logistic Regression
Use simple training data to predict if a person is a union member or not.
- Plots of 1-1 logistic regression to show the sigmoid shape; 
- Predictions of new data;
- Diagnostic plots;
- Deviance (likelihood ratio) test;
- Mention some measures of model fit.

We won't bother learning about the Hosmer-Lemeshow Statistic but will copy and paste if needed because there is no real explanation in the notes :(

[These slides](https://online.stat.psu.edu/stat462/node/207/) are a useful resource.
"""

# ╔═╡ 8e11ab83-2520-458c-8cee-bc44ade77992
begin
	default = dataset("ISLR", "Default")
	coerce!(default, :Default => OrderedFactor{2}, :Student => OrderedFactor{2})
	first(default, 2)
end

# ╔═╡ 4eb909d9-2b34-4fd6-9dab-1a2ed500f3b1
begin
	hot  = OneHotEncoder()
	mach = fit!(machine(hot, default))
	default_enc = MLJ.transform(mach, default)
	default_enc = default_enc[:, Cols(x -> ~(contains(x, "No")))]

	default_x = select(default_enc, :Balance) |> Matrix
	default_y = default_enc.Default__Yes 

	formula = @formula(Default__Yes ~ 1 + Balance)
	
	m_linear   =  lm(formula, default_enc)
	m_logistic = glm(formula, default_enc, Binomial(), LogitLink())
end

# ╔═╡ ef4a32f1-cf1f-4207-b6da-b281a7b8b54a
coef(m_linear)

# ╔═╡ 3ad77d7f-d6ba-45dd-976e-e6068abbe15d
begin
	# Plot a subset of the 10_000 data.
	default_sample = vcat(
		groupby(default_enc, :Default__Yes)[1][1:100, :],
		groupby(default_enc, :Default__Yes)[2][1:100, :]
	)
	@df default_sample scatter(:Balance, :Default__Yes, 
		alpha = .8,
		m = :vline, 
		markersize = 10,
		label = "",
		xlabel = "Balance",
		ylabel = "Defaulted")
	hline!([0,1], label = "", line = :dash)

	b = range(0, 2500, 1000)
	d_linear   = GLM.predict(m_linear,   DataFrame([b], [:Balance]))
	d_logistic = GLM.predict(m_logistic, DataFrame([b], [:Balance]))
	
	plot!(b, [d_linear, d_logistic], label = ["linear" "logistic"], lw=3)
end

# ╔═╡ 4d79470e-6adc-4ead-9a6c-075943aae4eb
md"""
## Trade Union Dataset
"""

# ╔═╡ 24134208-c28e-4bc1-a51e-bd7ab30a638b
begin
	trade = DataFrame(CSV.File(homedir() * "/regression/data/trade.csv"))
	select!(trade, Not(:Column1))
	rename!(trade, "years.educ"       => "educ", 
	           	   "years.experience" => "experience",
                   "union.member"     => "member")
	# Fix scientific types
	coerce!(trade,
		:south          => Multiclass,
		:female         => Multiclass,
		# Note that member is treated as a continuous probability in [0,1].
		:member         => Continuous,
		:race           => Multiclass,
...
# ╔═╡ 9b43a327-c6e5-4356-90b8-acae0d90d9eb
function logistic_plot(x, y, df; kwargs...)
	scatter(df[!, x], df[!, y],
		    alpha = .8, m = :vline, markersize = 10,
			label = "", xlabel = x, ylabel = y; kwargs...)
	hline!([0, 1], label = "", line = :dash)
	xrange = range(minimum(df[!, x]), maximum(df[!, x]), 200)
	model = glm(
		hcat(ones(size(df)[1]), df[!, x]),
		df[!, y], 
		Binomial(), 
		LogitLink())
	plot!(xrange, 
		  GLM.predict(model, hcat(ones(200), xrange)),
		  label = "")
end;

# ╔═╡ e9494db1-fd2d-41d9-84fc-7d6c8b9fe1d5
plot(
	logistic_plot(:wage, :member, trade),
	logistic_plot(:age, :member, trade),
	size = (800, 500),
)

# ╔═╡ 23144e4e-b531-494d-916d-0c6f9beb2c24
# Fit a model using wage and age
trade_logistic = glm(@formula(member ~ age + wage), trade, Binomial(), LogitLink())

# ╔═╡ df9bec43-4f6d-401d-b406-e65de9c47903
# Predict membership for new data using library function
# (and annoyingly including an intercept manually...)
round(GLM.predict(trade_logistic, hcat([1, 56, 6.5]...))[1], digits=4)

# ╔═╡ f7d0eb12-10c3-43d0-ad12-8ab084fe57e2
# Predict by hand using the model coefficients.
begin
	β₀, β₁, β₂ = coef(trade_logistic)
	z = β₀ + β₁ * 56 + β₂ * 6.5
	round(exp(z) / (1 + exp(z)), digits = 4)
end
# It's less than .5, so they are probably not a member.

# ╔═╡ 28557a2e-2a57-4ea2-8e5d-958e439bdddd
begin
	y = trade.member
	π̂ = GLM.predict(trade_logistic)
	# Raw residuals
	r = y - π̂
	# Pearson residuals
	p = r ./ sqrt.(π̂ .* (1 .- π̂))
	function f(x, r, xlabel, ylabel = ""; kwargs...)
		scatter(x, p, 
				marker_z = trade.member .< .5, 
				xlabel = xlabel,
				ylabel = ylabel,
				cbar = false, label = "", alpha = .3) 
		hline!([0], line = :dash, label = "")
	end
	plot(
		f(trade.wage, p, "wage", "Pearson Residual"),
		f(trade.age, p, "age"),
		f(range(1, length(p)), p, "index"),
		f(π̂, p, "prediction"),
		size = (800, 900)
	)
	# TODO: Add different makers for correct/incorrect. 
	# TODO: Add spline.
	# TODO: Add legend.
end

# ╔═╡ e1048da9-40b7-4e64-9b46-ac5660835106
begin
	# Test the hypothesis 
	#    H0: All model coefficients (except possibly the intercept) are zero. 
	# against
	# 	 H1: At least one model coefficient is non-zero.
	d₀ = nulldeviance(trade_logistic) # ~ Chisq(N - 1) since there is only an intercept.
	d₁ = deviance(trade_logistic)     # ~ Chisq(N - 3) since there are 3 model parameters.
	d = d₀ - d₁                       # ~ Chisq(2)
	println("Critical Value: $(round(quantile(Chisq(2), .95), digits=4))")
	println("Observed:       $(round(d, digits=4))")
	# The observed value is larger than the critical value, so we reject the null.

	# Just for fun, calculate the deviance by hand.
	D = 2 * (sum(log.(1 ./ π̂[y .== 1])) + sum(log.(1 ./ (1 .- π̂[y .== 0]))))
	println("\nLibrary deviance: $(round(deviance(trade_logistic), digits=4))")
	println("By hand:          $(round(D, digits=4))")
end

# ╔═╡ f531a7db-f71d-4c48-af65-b47970cd86f5
# Some measures of model fit that I don't understand.
GLM.aic(trade_logistic), GLM.bic(trade_logistic)
...
manifest_format = "2.0"
project_hash = "dbcf86573daa8fb8aafa7462d3d0b89583f8235f"
...
git-tree-sha1 = "464fbbed57138928976615103da348df88e54f0a"

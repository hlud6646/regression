...
# ╔═╡ 292035b8-bee7-11ee-3d8d-eb722404d2f0
begin
	using CSV
	using DataFrames
	using GLM
...
# ╔═╡ bfdd99bc-4daf-4201-877a-50c71f648318
begin
	# Load the plants data.
	plants = DataFrame(CSV.File(homedir() * "/regression/data/plants_dried.csv"))
	# Drop the double index column.
	select!(plants, Not(:Column1))
	# Treat the group as a 3-valued class for "Control" "TreatmentA" and "TreatmentB" rather than an arbitrary string.
	coerce!(plants, :group => Multiclass{3})
end

# ╔═╡ 07aa576c-91e2-4573-bed0-782d1421afb5
scatter(plants.weight, plants.group, label = "", xlabel = "weight")

# ╔═╡ 4c170191-abf8-4918-8f6e-70fead7ee0ed
begin
	# Instantiate a one hot encoder.
	hot = OneHotEncoder()
	# Bind the encoder to the data source and fit it.
	mach = fit!(machine(hot, plants))
	# One hot encode the data.
	plants_enc = MLJ.transform(mach, plants)
	# Separate the response and the design matrix.
	plants_weight, plants_design = unpack(plants_enc, ==(:weight))
	# Peak at the the design matrix.
	first(plants_design, 3)
end

# ╔═╡ d39edaba-1dd6-4cf5-8d6b-f21b82994d7d
# Fit a linear model
m₁ = lm(Matrix(plants_design), plants_weight)
# The coefficients of the model are the within group means.

# ╔═╡ 9e724fbf-6e1a-4d5e-aea3-95f9d959bfa3
begin
	# Null model is that all groups have the same mean, so 
	m₀ = lm(@formula(weight ~ 1), plants)
	k  = 10              # 10 replicates per level
	j  = 3               # 3 levels
	n  = j * k
	f1 = (deviance(m₀) - deviance(m₁)) * (n - j) / deviance(m₁) / (j - 1)
	round(f1, digits = 4)
	# The F-statistic for the hypothesis that all group means are equal.
end

# ╔═╡ 7503e47c-41bc-4384-bff4-b85e4da504b9
md"""
# ANOVA / ANCOVA
First we look at a one factor ANOVA. Seeds were given one of two treatements and their weights measured later.  We test if the mean weight for either treatement is significantly different from the control group.  The notes barely look at ANCOVA at all, beyond saying 'type this in R' so I'm not going to bother here.

### 1.
The first forulation is ``$ E[Y_{jk}] = \mu_{j} $`` giving an f-statistic of 
$(round(f1, digits = 4)).  This is computed by actually fitting a linear model to the design matrix (ANOVA is a GLM after all) and comparing to a null model.  But for some reason the library f-test doesn't work with this.

### 2. 
Prepending a column of ones to the design matrix gives model coefficients which represent the level deviations from the overall mean.  This model has perfect colinearity though so you can't really fit a linear model any more. 

### 3. 
Finally we use ``$ E[Y_{jk}] = \mu + α_j , \ \ \ α_1 = 0$`` where (from the notes)
> μ represents the effect of the first level and αⱼ measures the difference between the first level and the j-th level of the factor. 
This works the best and gives the most interpretable f-test.  The F-stat agrees with the manual calculation from part 1.

"""

# ╔═╡ d60fa11e-2f2b-47f5-8952-229df91b2bd2
# ftest(m₁)
# This doesn't work for some reason.

# ╔═╡ f63482b6-5666-45cb-9dd3-f8025842d5fe
# Tbh the notes are so thin on this that I feel like we're not supposed to have a working knowledge of it.  Anyway the third version works the best.

# ╔═╡ d9ea0e06-15c7-466b-a652-e505e3c2e7ae
m2 = lm(
	hcat(ones(n), plants_design[:, [2,3]]) |> Matrix,
	plants_weight
)

# ╔═╡ 105cd63a-08bb-41f0-8d2e-314babd5f127
ftest(m2)
# Same f-stat as above.
...
manifest_format = "2.0"
project_hash = "9ba38eae5be796bf8bcd791269659ba655612478"
